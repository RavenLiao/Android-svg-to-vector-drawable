...
Gradle runs on JDK 17 or newer; the release JAR is invoked with a separately installed Java 11 executable. The Linux job materializes and compares the full locked PNG corpus. macOS and Windows run only the committed minimal corpus to prove the Java 11 JAR process contract. No workflow in this repository publishes a release.
